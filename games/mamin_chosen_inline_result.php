...
if(empty($data['playing_chatters'][$chat_id]['mamin']['players'][$from_id])){
    ...
    if (empty($data['playing_chatters'][$chat_id]['mamin']['players'][$from_id]['jawab'])) addUserPlayingTime($from_id, 30, true);
    $data['playing_chatters'][$chat_id]['mamin']['players'][$from_id]['jawab'] = $jawaban;
}

$ada_yang_belum = false;
foreach($data['playing_chatters'][$chat_id]['mamin']['players'] as $k=>$v){
    if(empty($v['jawab'])){
        $ada_yang_belum = true;
        break;
    }
...
if(!$ada_yang_belum){
    $data['playing_chatters'][$chat_id]['mamin']['waktu_habis_sisa'] = 0;
...
KirimPerintah('sendMessage',[
    ...
    'parse_mode'=>'HTML',
    'reply_to_message_id'=> $data['playing_chatters'][$chat_id]['mamin']['soal_msgid'],
]);